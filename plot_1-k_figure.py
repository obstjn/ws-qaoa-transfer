import matplotlib.pyplot as plt
import os

from plotting import *


# Load files
grid_files = sorted(os.listdir('./test-run/energies'))
files = [None]*5
figsize = [(7.166, 7.166/2)]*5
figsize[4] = (7.166, 7.166)

ordering = [(0,1),
            (0,2,1),
            (0,2,1)]
dimensions = [(1,2),
              (1,3),
              (1,4),
              (1,5),
              (2,3)]

for k in range(5):
    # (1-k)-0 graphs
    name_list = sorted(['./test-run/energies/' + name for name in grid_files if f'(1-{k+1})-0' in name])
    files[k]= name_list
    # For reorder purposes add None
    name_list.append(None)
    # reorder the files
    # files[k] = [name_list[i] for i in ordering[k]]
    # Plot
    # draw_multiple_landscapes_and_graphs(files[k], rows=dimensions[k][0], cols=dimensions[k][1], figsize=figsize[k])
    # plt.savefig(f'(1-{k+1})-0.pdf')

# show
# plt.show()

graph_files = sorted(os.listdir('./test-run/graphs'))

for file in graph_files:
    G = nx.read_adjlist('./test-run/graphs/' + file, nodetype=int)
    warmstartings = get_relevant_warmstartings(G)
    print(f'{file[:7]}: {len(warmstartings)}')